...
function App() {
  ...
  return (
    <div className="min-h-screen bg-whiteg-zinc-900 text-zinc-800ext-zinc-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="mb-6">
          <div className="relative max-w-md">
            <svg
              className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400ext-zinc-500 pointer-events-none"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <input
              ...
              placeholder="搜索产业..."
              className="w-full pl-10 pr-4 py-2 text-sm rounded-lg border border-zinc-300order-zinc-600 bg-whiteg-zinc-800 text-zinc-900ext-zinc-100 placeholder-zinc-400laceholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-amber-400ocus:ring-amber-500 focus:border-transparent transition-shadow"
            />
            {query && (
              <button
                onClick={() => setQuery("")}
                className="absolute right-2 top-1/2 -translate-y-1/2 w-5 h-5 flex items-center justify-center rounded-full text-zinc-400 hover:text-zinc-600over:text-zinc-300 hover:bg-zinc-200over:bg-zinc-700 text-xs cursor-pointer"
              >
                ✕
              </button>
            )}
          ...
          {isFiltered && (
            <p className="mt-1.5 text-xs text-zinc-500ext-zinc-400">
        ...
        <footer className="mt-16 pt-8 border-t border-zinc-200order-zinc-700 text-xs text-zinc-400ext-zinc-500 space-y-1">
          <p>{meta.disclaimer}</p>
          <p>
