...
export function ComparisonView({ industry }: { industry: Industry }) {
  ...
  return (
    <section className="mb-12">
      <h2 className="text-lg sm:text-xl font-semibold mb-2 text-zinc-900ext-zinc-100">
        原文对比 · {industry.name}
      </h2>
      <p className="text-sm text-zinc-500ext-zinc-400 mb-4">
        {industry.summary}
      </p>

      {tooltip && (
        <div className="mb-4 p-3 sm:p-4 bg-zinc-50g-zinc-800 rounded-lg border border-zinc-200order-zinc-700 text-sm">
          ...
          <p className="text-zinc-600ext-zinc-400">{tooltip.reason}</p>
        </div>
      )}

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* 十四五 */}
        <div className="border border-zinc-200order-zinc-700 rounded-lg p-4 sm:p-6">
          <h3 className="text-sm font-semibold text-zinc-500ext-zinc-400 mb-3 uppercase tracking-wide">
            十四五规划纲要（2021-2025）
          </h3>
      ...
      {/* Highlights legend */}
      <div className="mt-4 flex flex-wrap gap-3 text-xs text-zinc-500ext-zinc-400">
...
function ParagraphWithHighlights({
  ...
  if (highlights.length === 0) {
    return <p className="text-zinc-700ext-zinc-300 mb-1">{text}</p>;
  }
  ...
  return (
    <p className="text-zinc-700ext-zinc-300 mb-1">
      {parts.map((part, i) =>
        part.highlight ? (
          <span
            key={i}
