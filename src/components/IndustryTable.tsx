...
function renderRows(
  ...
  return rows.map(({ ind, sig }) => {
    ...
    return (
      ...
      >
        <td className="px-2 py-1 font-medium text-zinc-900 whitespace-nowrap text-xs">
          {ind.name}
        </td>
    ...
    <section className="mb-12">
        ...
        <details className="mb-3">
          ...
          <div className="mt-2 space-y-1">
            ...
            <p className="text-xs text-zinc-400">
              ...
              映射精度分三档：<span className="text-emerald-500 font-medium">精准匹配</span>（有独立行业ETF，如芯片ETF→半导体）、
              <span className="text-amber-500 font-medium">相关匹配</span>（借用了相近行业ETF，如军工ETF→深海经济）、
              暂无对应（该产业处于早期或过于宽泛，无合适标的）。
            </p>
            <p className="text-xs text-zinc-400">
              <span className="text-amber-500">*</span> 标注的涨跌幅来自<span className="font-medium">代理ETF</span>——即这只ETF并非为该产业设计，其价格变动反映的是另一个行业板块的行情。
      ...
      <div className="overflow-x-auto rounded-lg border border-zinc-200">
        <table className="min-w-[640px] w-full text-sm">
          <thead>
            <tr className="border-b-2 border-zinc-300 bg-zinc-50 text-[11px] font-semibold text-zinc-500 uppercase tracking-wide">
...
function TierSection({
  ...
  return (
    <>
      <tr className={`border-b border-zinc-200 ${colorClass.split(" ")[0]} ${colorClass.split(" ")[1]}`}>
...
function StrategyLegend() {
  return (
    <details className="mt-6 group">
      <summary className="text-sm font-semibold text-zinc-500 cursor-pointer hover:text-zinc-700 transition-colors select-none">
        策略框架说明
      </summary>
